...
export async function collectKoelnProfileUrls(listUrl: string, limit?: number): Promise<string[]> {
  const html = await fetchHtml(listUrl)
  const $ = load(html)
  // Koelnmesse detail pages often include "/exhibitor/<slug>/"
  const links = new Set<string>()
  $('a[href]').each((_, el) => {
    const href = $(el).attr('href') || ''
    if (/\/exhibitor\/[A-Za-z0-9_-]+\/?(\?.*)?$/i.test(href)) {
...
export async function scrapeKoelnProfile(profileUrl: string): Promise<ExhibitorRecord> {
  ...
  $('a[href]').each((_, el) => {
    const href = $(el).attr('href') || ''
    if (/^https?:\/\//.test(href) && !/koelnmesse|exhibitor\//i.test(href)) {
  ...
  const contact_person_name = $('.contact-person, .person, .contact').find('.name, strong').first().text().trim() || null
  const contact_person_role = $('.contact-person, .person, .contact').find('.role, em, i').first().text().trim() || null
  ...
  if (!company_email) {
    ...
    const filtered = allMatches
      .filter(e => !/koelnmesse\.de/i.test(e))
...
export async function scrapeKoelnExhibitors(listUrl: string, limit?: number): Promise<ExhibitorRecord[]> {
  const urls = await collectKoelnProfileUrls(listUrl, limit)
  ...
  async function next() {
    ...
    const url = urls[i++]
    try {
      const rec = await scrapeKoelnProfile(url)
      results.push(rec)
    } catch (err) {
      console.error('Koeln profile scrape error:', err)
    }
    await next()
  }
