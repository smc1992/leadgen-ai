...
import { scrapeMuenchenExhibitors } from '@/lib/scrapers/messe-muenchen'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isSupabaseConfigured || !supabaseAdmin) {
      console.error('Supabase admin client not configured for Messe München import')
    ...
    const exhibitors = await scrapeMuenchenExhibitors(listUrl, limit)
    ...
    if (error) {
      console.error('Messe München import insert error:', error)
    ...
    console.error('Messe München import error:', error)
