...
import { scrapeMesseFrankfurtExhibitors } from '@/lib/scrapers/messe-frankfurt'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isSupabaseConfigured || !supabaseAdmin) {
      console.error('Supabase admin client not configured for AFAG import')
    ...
    // Scrape exhibitors from list
    const exhibitors = await scrapeMesseFrankfurtExhibitors(listUrl, limit)
    ...
    if (error) {
      console.error('AFAG import insert error:', error)
