...
import { scrapeBerlinExhibitors } from '@/lib/scrapers/messe-berlin'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isSupabaseConfigured || !supabaseAdmin) {
      console.error('Supabase admin client not configured for Messe Berlin import')
    ...
    const exhibitors = await scrapeBerlinExhibitors(listUrl, limit)
    ...
    const leadsPayload = exhibitors.map((ex) => ({
      ...
      score: 50,
      region: ex.company_country || 'germany',
      channel: 'event',
    ...
    if (error) {
      console.error('Messe Berlin import insert error:', error)
    ...
    console.error('Messe Berlin import error:', error)
